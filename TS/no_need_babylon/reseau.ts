    ...
    export module reseau{
        ...
        export class TriangulatedTriangle{
            ...
            go():Mamesh {
                ...
                let creator=new Regular2dPlus()
                creator.nbU=this.nbU+1
                creator.nbV=this.nbU+1
                creator.origin=new XYZ(0,0,0)
                creator.end=new XYZ(2,1,0)
                creator.putAVertexOnlyAtXYZCheckingThisCondition=(xyz)=>{
                    return (xyz.y<=xyz.x+0.0001) && (xyz.y<2-xyz.x+0.0001)
                }
                creator.maille=Maille.triangleH

                let mamesh=creator.go()
                if (! (this.origin.almostEqual(new XYZ(0,0,0))&& this.end.almostEqual(new XYZ(2,1,0)) )){

                    spacialTransformations.adjustInASquare(mamesh,this.origin,this.end)
                }
                ...
                if (this.markBorder) {
                    for (let v of mamesh.vertices){
                        if (v.links.length<=4) v.markers.push(Vertex.Markers.border)
                    }
                }
        ...
        export class TriangulatedTriangleOld{
        ...
        export class Regular2dPlus{
            ...
            go():Mamesh{
                ...
                let origin=new XYZ(this.origin.x,this.origin.y,0)
                let end=new XYZ(this.end.x,this.end.y,0)
                ...
                /**we place the mamesh according to the origine/end/lateral1/lateral2*/
                // if (this.lateral1==null && this.lateral2==null) {
                //
                //     let lateral1=new XYZ(end.x,origin.y,(origin.z+end.z)/2)
                //     spacialTransformations.affineTransformation_3vec(mamesh.vertices,origin,new XYZ(1,0,0),end,this.origin,lateral1,this.end)
                // }
                // else
                ...
                //
                // if (this.lateral1!=null && this.lateral2==null){
                //     spacialTransformations.affineTransformation_3vec(mamesh.vertices,new XYZ(this.origin.x,this.origin.y,0),new XYZ(1,0,0),new XYZ(this.end.x,this.end.y,0),this.origin,this.lateral1,this.end)
                // }
                // else if (this.lateral1==null && this.lateral2!=null){
                //     spacialTransformations.affineTransformation_3vec(mamesh.vertices,new XYZ(this.origin.x,this.origin.y,0),new XYZ(1,0,0),new XYZ(this.end.x,this.end.y,0),this.origin,this.lateral2,this.end)
                // }
                // else {
                //     spacialTransformations.quadTransformation_4vec(mamesh.vertices,new XYZ(this.origin.x,this.origin.y,0),new XYZ(1,0,0),new XYZ(this.end.x,this.end.y,0),new XYZ(0,1,0),this.origin,this.lateral1,this.end,this.lateral2)
                // }
