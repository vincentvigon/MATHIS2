...
module mathis{
    ...
    export module polyhedron{
        ...
        export class Polyhedron {
           ...
           dataToMamesh(data:{vertices: number[][], faces: number[][]}):Mamesh{
               ...
               for (let face of data.faces) {
                   ...
                   if (face.length == 3) {
                       ...
                       let faceVertex:Vertex[]=[mamesh.vertices[face[0]]]
                       /** For each face **/
           ...
           go(callback:(polyhedron:Mamesh)=>void=null): Mamesh {
                ...
                /**data here*/
                if (data!=null) {
                    let mamesh=this.dataToMamesh(data)
                    if (callback!=null) callback(mamesh)
                    return mamesh
                }
                /**data download */
                else{
                    if (callback==null) throw "the data for the polyhedron:"+this.type+"are not localy stored. They are downloaded, so you must give a callback method in the go-method"
                    var xhttp = new XMLHttpRequest();
                    let thus=this

                    xhttp.onreadystatechange = function() {
                        if (this.readyState == 4 && this.status == 200) {
                            let mamesh=thus.dataToMamesh(JSON.parse(this.responseText))
                            callback(mamesh)
                        }
                    };
                    let str=thus.type.replace(/\s+/g, '-').toLowerCase()
