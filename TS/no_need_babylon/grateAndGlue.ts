...
module mathis{
    ...
    export module grateAndGlue{
        ...
        import removeFromArray = mathis.tab.removeFromArray;
        ...
        export class VeryCloseVerticesFinder{
            nbBoxes=new XYZ(20,20,20)
            OUT_boxSize:XYZ
            doubleCheck=true

            addRepresentativeInRes=true
            ...
            go ():HashMap<Vertex,Vertex[]>{
                ...
                let boxMaker=new DisjointPackMaker(this.vertices)
                boxMaker.nbBoxes=this.nbBoxes

                let boxes0=boxMaker.go(new XYZ(-0.1,-0.1,-0.1))
                ...
                for (let box of boxes0) {
                        for (let i=1;i<box.length;i++) unionFind.union(box[0],box[i])
                }


                if (this.doubleCheck){
                    let boxes1=boxMaker.go(new XYZ(0.5,0.5,0.5))
                ...
                //
                //
                // let toSort=(a:Vertex[],b:Vertex[])=>{
                //     /**on classe par ordre croissant de hashNumber*/
                //     let diff=a[0].hashNumber-b[0].hashNumber
                //     if (diff!=0) return diff
                //     /**en cas d'égalité, on met le plus gros packet en premier*/
                ...
                // while (i<bbb.length){
                //     res.push(bbb[i])
                //     cc("-----",bbb[i][0].hashNumber,bbb[i].length)
                //     let j=i+1
                //     while (j<bbb.length && bbb[j][0].hashNumber==bbb[i][0].hashNumber){
                //         cc(bbb[j][0].hashNumber,bbb[j].length)
                //         j++
                //     }
                //     i=j
                // }
        ...
        export class DisjointPackMaker{
            nbBoxes=new XYZ(20,20,20)
            OUT_boxSize:XYZ
            ...
            preGo(){
                ...
                this.OUT_boxSize=XYZ.newFrom(amplitude)
            ...
            go (relativeDecay=new XYZ(0,0,0)):Vertex[][]{

                if (!this.preGoDone) this.preGo()


                let decay=XYZ.newFrom(relativeDecay).multiply(this.OUT_boxSize)

                let dico=new HashMap<XYZ,Vertex[]>(true)

                for (let vertex of this.vertices){
                    let val=vertex.position
                    let key=new XYZ(0,0,0)
                    ...
                    if(dico.getValue(key)==null) dico.putValue(key,[vertex])
                    else dico.getValue(key).push(vertex)
                }
            ...
            private floatToIndex(d,gauche,droite,nbBaton):number{
                /** attention, il faut *nbBaton et pas *(nbBaton-1).
                 *  Par contre, pour d==droite il faut faire un cas particulier si on veut que les indices aillent de 0 à nbBaton-1
                 */
                if (d>=droite) return nbBaton-1
                if (d<=gauche) return 0
                if ((droite - gauche)==0) return 0
        ...
        export class BoxesMaker{

            nbBoxes=new XYZ(20,20,20)
            OUT_boxSize:XYZ
            maxDistToBeClose:number=null
            //marginProp=new XYZ(0.5,0.5,0.5)
            ...
            go ():Vertex[][]{
                ...
                for (let entry of dico.allEntries()){
                    ...
                    if (entry.value.length>1){
                        ...
                        for (let i of [-1,0,1]){
                            for (let j of [-1,0,1]){
                                for (let k of [-1,0,1]){
                                    let keyDec=XYZ.newFrom(key).add(new XYZ(i,j,k))
        ...
        export class MergerNew {
            ...
            mapFromSource:HashMap<Vertex,Vertex>
            mapFromReceiver:HashMap<Vertex,Vertex[]>


            constructor(private mameshes:Mamesh[]) {
            ...
            goChanging():Mamesh {
                ...
                for (let mamesh of this.mameshes) allVertices=allVertices.concat(mamesh.vertices)



                this.mapFromReceiver= new VeryCloseVerticesFinder(allVertices).go()
                this.mapFromSource=new HashMap<Vertex,Vertex>()
                for (let entry of this.mapFromReceiver.allEntries()){
                    for (let v of entry.value) this.mapFromSource.putValue(v,entry.key)
                }
                ...
                let res=new Mamesh()
                /**on ne met que les vertex qui ne sont pas source (ceci seront poussés vers un receveur)*/
                for (let v of allVertices) if (this.mapFromSource.getValue(v)==null) res.vertices.push(v)




                if (this.mergeLink) this.letsMergeLinks(res,allVertices)


                //TODO if (this.mergeTrianglesAndSquares) this.letsMergeTrianglesAndSquares()


                //TODO if (this.mergeSegmentsMiddle) this.mergeCutSegment()
            ...
            private letsMergeLinks(mamesh:Mamesh,allVertices:Vertex[]):void {


                /**tous les vertex définif dont on a changé un lien = tous les receiver + tous les vertex qui touche un receiver */
                let verticesConcerning=new HashMap<Vertex,boolean>(true)
                for (let receiver of this.mapFromReceiver.allKeys()) verticesConcerning.putValue(receiver,true)


                /**ETAPE 1
                 * dès qu'un lien pointe vers une source, il est re-dirigé vers le receiver correspondant*/
                for (let vertex of allVertices){
                    for (let link of vertex.links) {
                        let perhapsAReceiver=this.mapFromSource.getValue(link.to)
                ...
                /**ETAPE 2 :
                 *
                 * tous les links d'une source sont recopiés dans le receiver correspondant
                 *
                 * */

                for (let source of this.mapFromSource.allKeys()){
                ...
                /**ÉTAPE 3: il faut supprimer tous les doubles liens que l'on a ainsi créer*/
                for (let vertex of verticesConcerning.allKeys()){
                    let dico=new HashMap<Vertex,boolean>()
                    let newLinks:Link[]=[]
                    for (let link of vertex.links) {
            ...
            private letsMergeTriangleQuadFaces(){
        ...
        export class Merger {
            ...
            constructor(receiverMamesh:Mamesh, sourceMamesh?:Mamesh,map?:HashMap<Vertex,Vertex[]>) {
                ...
                //if (receiverMamesh.vertices.length!=0) {
            ...
            goChanging():void {
                ...
                // /**cas d'un receiver vide*/
                // if (this.receiverMamesh.vertices.length==0){
                //     this.receiverMamesh=this.sourceMamesh
                //     cc("ici")
                //     return
                // }
